...
class preparetrain:
    def __init__(self, trainpath, splitcount, spl=False):
        self.tokenizer = None
        self.rawdata = []
        self.prepared_line = []
        self.prepared_label = []
        ...
        with open(trainpath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if spl == True and count == splitcount:
                    break
        self.prepare()
    ...
    def save2pkl(self, outputpath):
        ...
        with open(outputpath + '/trainlabel.json', "w") as f:
            ...
            f.write(json.dumps(
                {'prepared_label': self.prepared_label.tolist()}))
...
def main():
    ...
    outputpath = './data'
    preparetrain(trainpath, splitcount, True).save2pkl(outputpath)
