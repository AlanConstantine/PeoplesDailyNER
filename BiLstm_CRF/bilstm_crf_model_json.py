...
class LoadData(object):
    def __init__(self):
        self.traindata = np.array(self.loadjson(
            './data/traindata.json')['prepared_line'])
        self.trainlabel = np.array(self.loadjson(
            './data/trainlabel.json')['prepared_label'])
        self.word_index = self.loadjson('./data/word_index.json')
...
class nn:
    ...
    def trainingModel(self):
        ...
        result = model.fit(X_train, Y_train, batch_size=batchSize, epochs=5)

        model.save(
            'epoch_150_batchsize_100.h5')
